...
def results(request):
    if request.method == 'GET':
        ...
        # product = get_object_or_404(Product, name__startswith=text).first()
        if product.nutrigrade == 'a':
            ...
            if product.nutrigrade == 'b':
                ...
                # substitute_queries = get_object_or_404(Product, category=product.category, nutrigrade='a')
            elif product.nutrigrade == 'c':
                substitute_queries = Product.objects.filter(Q(category=product.category, nutrigrade='a')\
                     | Q(category=product.category, nutrigrade='b'))
                # substitute_queries = get_object_or_404(Product, Q(category=product.category, nutrigrade='a')\
                #      | Q(category=product.category, nutrigrade='b'))
            else:
                substitute_queries = Product.objects.filter(Q(category=product.category, nutrigrade='a')\
                     | Q(category=product.category, nutrigrade='b') | Q(category=product.category, nutrigrade='c'))
                # substitute_queries = get_object_or_404(Product, Q(category=product.category, nutrigrade='a')\
                #      | Q(category=product.category, nutrigrade='b') | Q(category=product.category, nutrigrade='c'))
